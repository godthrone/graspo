...
def _to_jsonable(value: Any) -> Any:
    """Recursively convert any value to a JSON-serializable type.

    Duck-typing checks for torch.Tensor are reasonable here — this is not
    probing an interface contract, but handling various tensor types
    (CPU/GPU/different dtypes) that may appear at runtime.
    """
    if isinstance(value, dict):
        return {str(key): _to_jsonable(child) for key, child in value.items()}
    if isinstance(value, (list, tuple)):
        return [_to_jsonable(child) for child in value]
    # Duck-typing for tensor capabilities, not interface contract probing
