...
Qwen3.5/Qwen3.6 的 text config 已能识别，vision weights 不进入训练范围；但包含 hybrid `linear_attention` 的 checkpoint 会在 kernel 实现前明确失败，不会用近似的 Qwen3 full-attention 层悄悄训练。
