...
GRASPO 默认安装保持轻量，smoke 路径不会下载完整 PyTorch GPU 栈。准备真正训练模型时再显式安装
训练依赖：
...
`train` extra 中把 PyTorch 固定在 `2.5.x` 系列，因为一些长期运行的 Linux GPU 服务器仍是
...
```bash
# 只有 spreadsheet 输入需要 data extra。
