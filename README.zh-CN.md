...
GRASPO 是一个基于 GRPO 改进的强化学习算法，面向可以做结构化校验的语言模型任务，例如
JSON 生成、信息抽取、分类、表单解析和 tool-call 参数生成。

相比通用 GRPO 训练，GRASPO 增加了 rollout retry、perfect-answer skip、invalid group
过滤、no-preference-gap 过滤、completion-level ReplayBuffer 训练，以及 readable reward/debug
日志。这些改进让它更适合格式化输出场景，因为模型回复可能是格式错误、部分正确、完全正确，
或者只是缺少几个关键字段。

## 为什么需要 GRASPO

GRPO 适合一组 sampled completion 内存在有效 reward 差异的场景。结构化输出任务会额外遇到几类问题：
...
- 长格式化答案可能被截断，如果不保存实际模型输出，很难 debug reward 为什么低。

GRASPO 保留 GRPO 在 rollout group 内比较 completion 的核心思想，同时加入面向结构化输出的过滤、
重试和 replay 机制，让训练集中在真正能教会模型的样本组上。
